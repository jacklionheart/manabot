...
ManaCost ManaCost::parse(const std::string& mana_cost_str) {
    ...
    while (i < mana_cost_str.length()) {
        ...
        if (std::isdigit(c)) {
            ...
            switch (c) {
                case 'W': mana_cost.cost[Color::WHITE] += 1; break;
                case 'U': mana_cost.cost[Color::BLUE] += 1; break;
                case 'B': mana_cost.cost[Color::BLACK] += 1; break;
                case 'R': mana_cost.cost[Color::RED] += 1; break;
                case 'G': mana_cost.cost[Color::GREEN] += 1; break;
                case 'C': mana_cost.cost[Color::COLORLESS] += 1; break;
                default:
...
std::string ManaCost::toString() const {
    ...
    for (const auto& [color, quantity] : cost) {
        for (int i = 0; i < quantity; i++) {
            char symbol;
            switch (color) {
                case Color::COLORLESS: symbol = 'C'; break;
                case Color::WHITE:     symbol = 'W'; break;
                case Color::BLUE:      symbol = 'U'; break;
                case Color::BLACK:     symbol = 'B'; break;
...
bool Mana::canPay(const ManaCost& mana_cost) const {
    ...
    for (const auto& [color, required] : mana_cost.cost) {
        if (temp_mana[color] < required) {
            return false;
        }
        temp_mana[color] -= required;
    }

    // Pay generic mana costs
    int generic_needed = mana_cost.generic;
    int available_mana = 0;
    for (const auto& [color, amount] : temp_mana) {
