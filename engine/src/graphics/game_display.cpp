...
GameDisplay::GameDisplay(Game* game) 
    : game(game),
      window(sf::VideoMode(1024, 768), "Magic Game"),
      space_pressed(false) {
...
GameDisplay::LayoutMetrics GameDisplay::calculateLayout() const {
    ...
    float remaining_height = metrics.window_height - (
        metrics.info_section_height + 
        metrics.top_hand_height + 
        metrics.bottom_hand_height
    );
...
void GameDisplay::drawCard(const Card* card, float x, float y, float width, float height,
    ...
    drawRoundedRect(cardBounds, fillColor, gradientColor,
                   height * theme.CARD_CORNER_RADIUS,
...
void GameDisplay::drawPermanent(const Permanent* permanent, float x, float y, 
                              float width, float height) {
    sf::Color fillColor = permanent->card->types.isLand() ? 
        theme.LAND_FILL : theme.CREATURE_FILL;
    sf::Color gradientColor = permanent->card->types.isLand() ? 
        theme.LAND_GRADIENT : theme.CREATURE_GRADIENT;
...
void GameDisplay::drawHand(Player* player, const sf::FloatRect& bounds) {
    ...
    float total_cards_width = (hand_cards.size() * layout.card_width) + 
                            ((hand_cards.size() - 1) * layout.spacing);
...
void GameDisplay::drawBattlefield(Player* player, const sf::FloatRect& bounds) {
    ...
    drawRoundedRect(bounds, theme.SECTION_BACKGROUND, 
                   sf::Color(theme.SECTION_BACKGROUND.r, 
                           theme.SECTION_BACKGROUND.g,
    ...
    separator.setPosition(bounds.left + bounds.width * 0.05f, 
                         bounds.top + landHeight);
    ...
    sf::FloatRect creature_bounds(bounds.left, bounds.top + land_section_height, 
                                bounds.width, creature_section_height);
...
void GameDisplay::drawPermanentRow(const std::vector<const Permanent*>& permanents, 
    ...
    if (permanents.empty()) return;
    
    float total_width = (permanents.size() * layout.card_width) + 
                       ((permanents.size() - 1) * layout.spacing);
...
void GameDisplay::render() {
    ...
    drawLifeTotals(sf::FloatRect(0, layout.getInfoSectionY(), 
                                layout.window_width, layout.info_section_height));
    
    drawHand(game->players[1].get(), sf::FloatRect(0, layout.getTopHandY(), 
                                              layout.window_width, layout.top_hand_height));
    drawBattlefield(game->players[1].get(), sf::FloatRect(0, layout.getTopBattlefieldY(), 
                                                     layout.window_width, layout.top_battlefield_height));
    drawBattlefield(game->players[0].get(), sf::FloatRect(0, layout.getBottomBattlefieldY(), 
                                                     layout.window_width, layout.bottom_battlefield_height));
    drawHand(game->players[0].get(), sf::FloatRect(0, layout.getBottomHandY(), 
